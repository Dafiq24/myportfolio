...
#### (07.50 - 08.10 9/24/2026)
Finalisasi Tugas Individu 4 dilakukan dengan audit berlapis terhadap kode, database, dependency, browser, Git, dan deployment. `python manage.py check` tidak menemukan masalah, `python manage.py makemigrations --check` menyatakan tidak ada perubahan model yang belum direkam, dan `showmigrations` menunjukkan migration `0001` sampai `0006` sudah diterapkan. Seluruh 86 test lulus pada database test terpisah. `python -m pip check` yang dijalankan melalui virtual environment proyek menghasilkan `No broken requirements found`, sedangkan `git diff --check` tidak menemukan whitespace error selain informasi normalisasi line ending Windows.

Audit PWS dilakukan kembali setelah seluruh commit kode dan dokumentasi chapter sebelumnya dikirim ke GitHub serta PWS. Halaman publik, autentikasi, Star/Unstar, JSON, safe post-login redirect, custom 403, serta kontrol CRUD diuji. Pengunjung hanya memperoleh akses baca; user biasa dapat memberi star tetapi menerima 403 pada CRUD; akun dalam grup Editor dapat memperbarui Experience tetapi tidak membuat atau menghapusnya; superuser dapat menjalankan seluruh operasi. Objek uji sementara tidak dipertahankan, endpoint JSON tidak menampilkan identitas pemberi star, dan tidak ditemukan error 500. Grup dan membership Editor dibuat pada Django Admin PWS karena data autentikasi deployment terpisah dari SQLite lokal.

Salah satu pemeriksaan browser juga memperlihatkan halaman `127.0.0.1:8000` tanpa styling setelah server lokal dihentikan. Kondisi tersebut bukan bukti bahwa template Django dapat berjalan tanpa server. Pemeriksaan port menunjukkan tidak ada listener pada port 8000 dan request baru ditolak; browser hanya mempertahankan dokumen sebelumnya di memori, sementara stylesheet dengan versi cache baru gagal dimuat. Temuan ini dicatat agar screenshot atau tampilan cache tidak dianggap sebagai bukti server aktif. Pengujian fungsional selalu dilakukan ketika server lokal berjalan atau melalui deployment PWS.
...
Codex digunakan secara bertahap untuk membaca ketentuan, memetakan pola Tutorial 04 ke Experience, menjelaskan konsep, membantu implementasi, menyusun test, memeriksa diff, dan menyiapkan dokumentasi. AI tidak diberi satu prompt untuk menghasilkan seluruh tugas. Setiap tahap dibatasi pada satu tujuan yang dapat diuji, kemudian hasil terminal atau browser menjadi masukan untuk tahap berikutnya. Saya tetap memilih Experience sebagai objek tugas, menentukan desain yang dipertahankan, membuat akun dan grup pengujian, menjalankan migration serta command lokal, menguji empat role, mengelola commit dan push, serta memutuskan apakah hasil layak dipertahankan.

Strategi prompting mengikuti urutan **konteks, tujuan, ketentuan teknis, batasan, keluaran, dan verifikasi**. Batasan seperti “jangan mengubah model atau migration yang sudah benar”, “jangan mengandalkan template sebagai keamanan”, dan “tolak tujuan redirect eksternal” digunakan untuk mencegah perubahan melebar. Acceptance criteria ditulis dalam bentuk respons HTTP, perubahan database, elemen UI, serta test yang dapat diperiksa. Log berikut merangkum prompt yang benar-benar digunakan; password, cookie, kredensial deployment, dan data sensitif tidak dicantumkan.

| Tahap | Prompt atau tindak lanjut utama | Hasil AI | Verifikasi dan keputusan manual |
| --- | --- | --- | --- |
| Fondasi star | “Tambahkan relasi Many-to-Many dari Experience ke user memakai `settings.AUTH_USER_MODEL`, `related_name` yang jelas, dan `blank=True`. Buat migration tanpa mengubah data lama; jangan buat view atau UI dahulu.” | Menambahkan `Experience.starred_by` dan migration `0006`. | Migration check dan system check bersih; 70 test lama lulus sebelum fitur dilanjutkan. |
| Toggle dan UI | “Model sudah memiliki `starred_by`; jangan ubah model atau migration. Tambahkan UUID route, POST-only view, login, CSRF, status Star/Unstar, jumlah total, JSON aman, dan test keamanan.” | Menambahkan toggle, komponen timeline, styling, whitelist JSON, serta test redirect, 403, 404, 405, uniqueness, dan UI. | Pengujian browser dua akun dan 78 test memastikan star independen serta identitas user tidak masuk JSON. |
| Matriks role | “Terapkan anonymous, user biasa, Editor, dan superuser. Editor hanya update; superuser create/update/delete; seluruh user login tetap boleh star. Lakukan server-side check dan sembunyikan kontrol UI.” | Membuat helper role, melindungi view CRUD, dan merender kontrol sesuai hak akses. | Grup `Editor` dibuat melalui Admin; akses langsung dan tombol diuji untuk empat role; 82 test lulus. |
| UX keamanan | “Hormati parameter `next` hanya bila URL lokal dan aman; tolak domain eksternal. Buat halaman 403 informatif tanpa mengekspos permission internal dan tambahkan security test.” | Menambahkan validasi `url_has_allowed_host_and_scheme`, hidden `next`, fallback, handler 403, template, dan styling responsif. | URL lokal, URL absolut eksternal, URL scheme-relative, login gagal, serta 403 diuji; seluruh 86 test lulus. |
| Audit final | “Audit checklist tugas, migration, dependency, test, PWS, privasi JSON, Git, dan dokumentasi. Bedakan bukti otomatis, browser, dan keterbatasan environment.” | Menyusun daftar pemeriksaan dan menemukan bahwa dependency check runtime pendamping berbeda dari virtual environment pengguna. | `pip check` di environment pengguna menyatakan tidak ada dependency rusak; empat role dan seluruh alur diuji ulang pada PWS. |

Keterbatasan AI muncul pada perbedaan environment eksekusi. Interpreter virtual environment proyek tidak dapat dijalankan langsung oleh proses pendamping karena menunjuk instalasi Python Windows yang tidak tersedia pada konteks tersebut. AI memakai runtime Python bawaan dengan `site-packages` proyek untuk menjalankan Django, sedangkan saya mengulang pemeriksaan melalui terminal `(env)`. Migration yang dibuat tetap diverifikasi oleh `makemigrations --check`. Pada audit dependency, runtime pendamping melaporkan paket binary tidak didukung platformnya; hasil tersebut tidak disamakan dengan kondisi proyek. Pemeriksaan ulang menggunakan `python -m pip check` pada environment asli menghasilkan `No broken requirements found`.

AI juga tidak dapat menyimpulkan keadaan deployment hanya dari test lokal. User, grup Editor, membership, session, dan data portofolio berada pada database environment masing-masing, sehingga konfigurasi lokal tidak otomatis membuktikan PWS benar. Saya membuat grup Editor pada Admin PWS dan menguji setiap role secara manual. Demikian pula, assertion HTML membuktikan keberadaan atau ketiadaan markup tetapi tidak membuktikan kualitas visual, focus state, cache static, atau seluruh perilaku browser. Pemeriksaan desktop, layar sempit, dan request aktual tetap dilakukan.
